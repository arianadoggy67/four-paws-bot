...
def склонение_единиц(кол_во, единица):
    ...
    # Ищем склоняемое слово ВО ВСЕЙ СТРОКЕ
    for ключ, формы in СКЛОНЕНИЯ.items():
        if ключ in единица:
            if 2 <= кол_во % 10 <= 4 and (кол_во % 100 < 10 or кол_во % 100 >= 20):
                return единица.replace(ключ, формы[0])
            else:
                return единица.replace(ключ, формы[1])
    
    # Если не нашли в словаре — общие правила для последнего слова
    слова = единица.split()
    последнее = слова[-1]
    склонённое = последнее
    
    if 2 <= кол_во % 10 <= 4 and (кол_во % 100 < 10 or кол_во % 100 >= 20):
        if последнее.endswith("ка"):
            склонённое = последнее[:-2] + "ки"
        elif последнее.endswith("а"):
            склонённое = последнее[:-1] + "ы"
        elif последнее.endswith("я"):
            склонённое = последнее[:-1] + "и"
        elif последнее.endswith("ь"):
            склонённое = последнее[:-1] + "и"
    else:
        if последнее.endswith("ка"):
            склонённое = последнее[:-2] + "ек"
        elif последнее.endswith("а"):
            склонённое = последнее[:-1] + ""
        elif последнее.endswith("я"):
            склонённое = последнее[:-1] + "й"
        elif последнее.endswith("ь"):
            склонённое = последнее[:-1] + "ей"
        else:
            склонённое = последнее + "ов"
    
    return единица.replace(последнее, склонённое)
