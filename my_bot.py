...
CHECK_TEXT = "🐾 Напиши название продукта, и я скажу, можно ли его твоему хвостику. А если хочешь персональную дозу — напиши в формате: Кличка, порода, вес, возраст, стерилизация (да/нет), продукт (можно через запятую или просто словами)."
...
def разобрать_свободный_запрос(текст):
    """
    Пытается извлечь из текста: кличку, породу, вес, возраст, стерилизацию, продукт.
    Понимает форматы:
    - Чарли чихуахуа 3 кг 3 года да яблоко
    - Чарли, чихуахуа, 3 кг, 3 года, да, яблоко
    - яблоко
    """
    текст = текст.strip()
    ...
    # Ищем продукт — последнее слово или словосочетание, которое есть в таблице
    слова = текст.split()
    
    # Пробуем найти продукт, начиная с последнего слова
    for i in range(len(слова) - 1, -1, -1):
        кандидат = " ".join(слова[i:])
        if найти_продукт(кандидат):
            # Всё, что до этого — данные о собаке
            данные_собаки = " ".join(слова[:i])
            # Разбиваем данные о собаке на части
            части_собаки = данные_собаки.split()
            
            # Ищем кличку (первое слово)
            кличка = части_собаки[0] if части_собаки else "Питомец"
            
            # Ищем породу
            порода = "чихуахуа"
            породы = ["чихуахуа", "чихуа", "мопс", "такса", "хаски", "лабрадор"]
            for п in породы:
                if п in данные_собаки.lower():
                    порода = п
                    break
            
            # Ищем вес
            вес = 2.5
            for j, слово in enumerate(части_собаки):
                if "кг" in слово.lower() or "г" in слово.lower() or слово.replace(".", "").isdigit():
                    try:
                        if "кг" in слово.lower():
                            вес = float(слово.lower().replace("кг", "").strip())
                        elif "г" in слово.lower():
                            вес = float(слово.lower().replace("г", "").strip()) / 1000
                        else:
                            вес = float(слово)
                    except:
                        pass
            ...
            # Ищем возраст
            возраст = "взрослая"
            if "месяц" in данные_собаки.lower() or "мес" in данные_собаки.lower():
                возраст = "щенок"
            elif "год" in данные_собаки.lower() or "лет" in данные_собаки.lower():
                цифры = ''.join(filter(str.isdigit, данные_собаки))
                if цифры:
                    лет = int(цифры)
                    if лет < 1:
                        возраст = "щенок"
            ...
            # Ищем стерилизацию
            стерилизована = False
            if "да" in данные_собаки.lower() or "yes" in данные_собаки.lower():
                стерилизована = True
            
            return [кличка, порода, str(вес), возраст, str(стерилизована), кандидат]
...
def handle_message(message):
    ...
    # Сначала пробуем умный разбор
    разбор = разобрать_свободный_запрос(текст)
    
    if разбор and len(разбор) >= 6:
        ...
        try:
            ...
            вес = СРЕДНИЙ_ВЕС_ЧИХУАХУА
        возраст = разбор[3]
        стерилизована = разбор[4].lower() in ["true", "да", "yes", "1"]
        продукт = разбор[5]
