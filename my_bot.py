...
А если хочешь персональную дозу — напиши мне данные о питомце и продукт. Например:

Чарли чихуахуа 3 кг 3 года да яблоко
(можно с запятыми или без)"""
...
def умный_разбор(текст):
    ...
    стоп_слова = ["да", "нет", "yes", "no", "кг", "г", "чихуахуа", "чихуа", "хуа", "месяц", "месяцев", "год", "года", "лет", "годик"]
    
    продукт = None
    for длина in range(min(4, len(слова)), 0, -1):
        кандидат = " ".join(слова[-длина:])
        if кандидат.lower() in стоп_слова:
            continue
        if найти_продукт(кандидат):
            продукт = кандидат
            слова_собаки = слова[:-длина]
            break
    
    if not продукт:
        return None
    
    кличка = слова_собаки[0] if слова_собаки else "Питомец"
    порода = "чихуахуа"
    вес = 2.5
    возраст = "взрослая"
    стерилизована = False
    
    for слово in слова_собаки:
        if "чихуа" in слово.lower():
            порода = "чихуахуа"
            break
    
    for i, слово in enumerate(слова_собаки):
        if "кг" in слово.lower():
            try:
                кг = float(слово.lower().replace("кг", "").strip())
                if i + 2 < len(слова_собаки) and "г" in слова_собаки[i + 2].lower():
                    г = float(слова_собаки[i + 2].lower().replace("г", "").strip())
                    вес = кг + г / 1000
                else:
                    вес = кг
            except:
                pass
            break
        elif "г" in слово.lower() and "кг" not in слово.lower():
            try:
    ...
    for слово in слова_собаки:
        if "месяц" in слово.lower() or "мес" in слово.lower():
            возраст = "щенок"
            break
        if "год" in слово.lower() or "лет" in слово.lower() or "годик" in слово.lower():
            цифры = ''.join(filter(str.isdigit, слово))
            if цифры:
                лет = int(цифры)
                if лет < 1:
                    возраст = "щенок"
    ...
    for слово in слова_собаки:
        if слово.lower() in ["да", "yes"]:
            стерилизована = True
            break
        if слово.lower() in ["нет", "no"]:
            стерилизована = False
            break
    
    return [кличка, порода, str(вес), возраст, str(стерилизована), продукт]
...
def handle_message(message):
    ...
    # 1. Пробуем умный разбор
    разбор = умный_разбор(текст)
    
    if разбор and len(разбор) >= 5:
        кличка = разбор[0]
        порода = разбор[1]
        try:
            вес = float(разбор[2])
    ...
    # 2. Пробуем простой поиск продукта
