...
def умный_разбор(текст):
    ...
    Возвращает список [кличка, порода, вес, возраст, стерилизация, продукт] или None.
    """
    ...
    # Ищем продукт — пробуем последние 1-3 слова
    продукт = None
    for длина in [3, 2, 1]:
        if len(слова) >= длина:
    ...
    for i, слово in enumerate(слова_собаки):
        if "кг" in слово.lower() or "г" in слово.lower():
            try:
                вес = разобрать_вес(слово)
            except:
                pass
            break
    ...
    # Ищем стерилизацию
    if "да" in " ".join(слова_собаки).lower() or "yes" in " ".join(слова_собаки).lower():
        стерилизована = True
