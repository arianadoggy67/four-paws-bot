...
def склонение_единиц(кол_во, единица):
    ...
    слова = единица.split()
    последнее = слова[-1]
    склонённое_последнее = последнее
    
    for ключ, формы in СКЛОНЕНИЯ.items():
        if ключ in последнее:
            if 2 <= кол_во % 10 <= 4 and (кол_во % 100 < 10 or кол_во % 100 >= 20):
                склонённое_последнее = формы[0]
            else:
                склонённое_последнее = формы[1]
            break
    
    if склонённое_последнее == последнее:
        if 2 <= кол_во % 10 <= 4 and (кол_во % 100 < 10 or кол_во % 100 >= 20):
            if последнее.endswith("ка"):
                склонённое_последнее = последнее[:-2] + "ки"
            elif последнее.endswith("а"):
                склонённое_последнее = последнее[:-1] + "ы"
            elif последнее.endswith("я"):
                склонённое_последнее = последнее[:-1] + "и"
            elif последнее.endswith("ь"):
                склонённое_последнее = последнее[:-1] + "и"
            elif последнее in ["пюре", "соцветие"]:
                склонённое_последнее = последнее
        else:
            if последнее.endswith("ка"):
                склонённое_последнее = последнее[:-2] + "ек"
            elif последнее.endswith("а"):
                склонённое_последнее = последнее[:-1] + ""
            elif последнее.endswith("я"):
                склонённое_последнее = последнее[:-1] + "й"
            elif последнее.endswith("ь"):
                склонённое_последнее = последнее[:-1] + "ей"
            elif последнее in ["пюре", "соцветие"]:
                склонённое_последнее = последнее
    
    результат = слова[:-1] + [склонённое_последнее]
...
def рассчитать_дозу(строка, вес_собаки, возраст, стерилизована, средний_вес_породы, порода_название):
    ...
    # ДИАГНОСТИКА СКЛОНЕНИЯ
    try:
        bot.send_message(YOUR_TELEGRAM_ID, f"ДИАГНОСТИКА: кол_во={кол_во}, единица={единица}, результат={единица_склон}")
    except:
        pass
